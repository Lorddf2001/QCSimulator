function QuOrderFinder()
    % Parameters for Order Finding: a^r mod N = 1
    a = 7; % The base
    N_mod = 15; % The modulus
    ...
    title(['Input Register Distribution (a=', num2str(a), ', N_{mod} = ' , num2str(N_mod), ' )']);
    ...
    for i = 1:length(found_peaks)
        ...
        if ~isempty(r_candidate)
            fprintf('Peak at %d matched to Order r = %d\n', k, r_candidate);
            fprintf('Verification: %d^%d mod %d = %d\n', a, r_candidate, N_mod, mod(a^r_candidate, N_mod));
            break;
        end
    end
